...
* DONE Week 11: ~data.frame~ practice / Lists / intro to ~ggplot2~

- [X] Home assignments (Nov 9):
  1) Review of data frames with a Fantasy Creatures dataset
  2) Review of base R plotting with a Census dataset (~uspop~)

- [X] If you have questions on your project: Prepare me! Or send me an
  email with your question.

- [X] New DataCamp lesson (Nov 14): EDA and Plotting Categories
  + Prerequisites: R stats and introduction to ~ggplot2~
  + Lesson has videos (yay!) and downloadable slides
  + Uses the "Tidyverse" language (data.frame -> "tibbles")
...
- [X] Continue the data frame practice: [[https://tinyurl.com/data-frame-ipynb][tinyurl.com/data-frame-ipynb]] -
  independent code along.

- [X] Pepquiz: DataCamp review - Lists and Plots
...
- [X] Introduction to ~ggplot2~ (Wednesday)

** Very quick introduction to ~ggplot2~

Open this notebook in datalab and *make a copy to code along*:

https://tinyurl.com/ggplot2-intro

Or download the raw .ipynb file and import it to datalab:

https://tinyurl.com/ggplot2-intro-ipynb

This is ready for you to run cell by cell.

** DataCamp review: Lists and Plots

Popquiz with solutions:

1. In R, what is a ~list~, and how does it differ from a vector in
   terms of data types and structure?
   #+begin_quote
   A ~list~ is a collection of objects that can have different data
   types and structures.
   #+end_quote

2. What function is used to create a list in R, and what types of
   objects can it contain?
   #+begin_quote
   It can contain numbers, characters, Booleans, vectors,
   matrices, data frames, or other lists, which themselves can
   contain other lists (no nesting limit).
   #+end_quote
   #+begin_src R :session *R* :results none :exports both
     l <- list(1, "hello world", c(100,200), Nile, options())
   #+end_src

3. If you have a list my_list with three components and want to assign
   names "vec", "mat", and "df" after creation, how would you do it?
   #+begin_src R :session *R* :results output :exports both
     l <- list(c(100,200,300), matrix(1:9,nrow=3), ToothGrowth)
     names(l) <- c("vec", "mat", "df")
     names(l)
   #+end_src
...
4. Given a ~list~ =movie_list= containing a character vector, a ~numeric~
   vector, and a data frame, how do you select the first list
   component?
   #+begin_src R :session *R* :results output :exports both
     ## Step 1: Create the vectors
     scores <- c(4.5, 5.0, 4.8)
     comments <- c("Great!", "Amazing!", "Loved it!")

     ## Step 2: Create the data frame of reviews
     movie_reviews <- data.frame(scores, comments)

     ## Step 3: Calculate the average score (question 9)
     avg_score <- mean(scores)

     ## Step 4: Create the movie_list (question 10)
     movie_list <- list(
       title = "Star Wars",             # character
       actors = c("Actor1", "Actor2"),  # vector
       reviews = movie_reviews,         # data.frame
       avg = avg_score                  # numeric
     )

     ## Print the list to verify
     movie_list

     ## Select first list component
     movie_list[[1]]
     movie_list[1][[1]]
   #+end_src
...
5. In a list =movie_list= with named components "title", "scores", and
   "reviews", how can you select the "reviews" component using two
   different methods?
   #+begin_src R :session *R* :results output :exports both
     movie_list$reviews
     movie_list[["reviews"]]
...
6. Suppose movie_list has a component that is a vector of actor names
   named "actors". How would you select the second actor from this
   component?
   #+begin_src R :session *R* :results output :exports both
     movie_list[["actors"]][2]
   #+end_src
...
7. You have a vector =scores <- c(4.5, 5.0, 4.8)= and a vector =comments
   <- c("Great!", "Amazing!", "Loved it!")=. How would you create a
   nested list named =movie_reviews= that contains these vectors as
   components named =ratings= and =feedback=, within a sublist named =data=?
   #+begin_src R :session *R* :results output :exports both
     ## Define the vectors
     scores <- c(4.5, 5.0, 4.8)
     comments <- c("Great!", "Amazing!", "Loved it!")
     ...
     movie_reviews2 <-                      # store list
       list(data =                         # `data` sublist
              list(ratings = scores,       # ratings component
                   feedback = comments))   # feedback component
     movie_reviews2
   #+end_src
   ...
   :
   : $data$feedback
   : [1] "Great!"    "Amazing!"  "Loved it!"

8. You have a list =movie_list <- list(title = "Star Wars", scores =
   c(4.5, 5.0, 4.8, 4.2, 4.9))=. How would you create a histogram of
   the =scores= component using base R?

   #+begin_src R :file ../img/movieHist.png :session *R* :results graphics output file :exports both
     movie_list2 <- list(title = "Star Wars", scores = c(4.5, 5.0, 4.8,
                                                         4.2, 4.9))
     hist(movie_list2$scores)
   #+end_src

   #+RESULTS:
   [[file:../img/movieHist.png]]

9. You have a list:
   #+begin_src R :session *R* :results output :exports both
     movies <- list(title = "Star Wars",
                    years =   c(2020, 2021, 2022, 2023, 2024),
                    ratings = c(4.5, 4.7, 4.8, 4.6, 4.9))
     movies
   #+end_src

   How would you create a line plot of ratings versus years (base R)?

   #+begin_src R :file ../img/moviesLine.png :session *R* :results graphics output file :exports both
     plot(movies$ratings ~ movies$years, type="b")
   #+end_src

   #+RESULTS:
   [[file:../img/moviesLine.png]]

10. If you have a list with two vector members runtime and ratings of
    differing lengths, can you create a scatter plot of ratings
    vs. runtime?

    #+begin_quote
    No, you cannot because ~plot~ requires its input vectors to be of
    equal length.
    #+end_quote

** Lists are tricky - an example

Here is an example problem:

- ~l~ is a ~list~ of five elements, one of which is the ~Nile~ time
  series. How can I get its ~list~ index?

- Where is ~Nile~ in the list?
  #+begin_src R :session *R* :results output :exports both
    str(l[[4]])
  #+end_src
...
- You might be tempted to write =which(l == Nile)= - worked for vectors
  and data frames, right?

  #+begin_src R :session *R* :results output :exports both
    which(l==Nile) # error message
  #+end_src
...
- To do this, you need more tools: ~sapply~ and lambda (anonymous)
  functions:

  #+begin_src R :session *R* :results output :exports both
    which(sapply(l, function(x) identical(x,Nile)))
  #+end_src
...
- Explanation:
  #+begin_quote
  1. =sapply(X,FUN)= applies the function =FUN= to =X=, going through
     element after element.
  2. =function(x)= is an anonymous (unnamed) function.
  3. =function(x)= tests if =x= is =Nile= or not.
  4. If the function returns ~TRUE~ when ~which~ returns the corresponding
     index value.

  This is stuff for DSC 205 Introduction to advanced data science!
  #+end_quote

* DONE Week 12: DataCamp: EDA - Categorical data

- [X] This week you'll be working on Exploratory Data Analysis in
  DataCamp, and on more and different plot types and datasets to plot
  in class. There is an overlap of perhaps 30% between these. All the
  DataCamp examples are using ~ggplot2~ and ~dplyr~, while mine use ~base R~
  and ~data.frame~, with the occasional ~ggplot2~ example for contrast.

- [X] There is no class meeting on Wed: Work on your term projects!

- [X] Home assignment based on this week:
  [[https://tinyurl.com/piccolo-problems-ipynb][tinyurl.com/piccolo-problems-ipynb]]
  + --> Download raw .ipynb file and upload it to a datalab notebook
  + --> Submit the URL of the completed file from datalab

- [X] Solutions to the "Simple plotting" exercise are in GitHub:
  [[https://tinyurl.com/simple-plotting-solutions][tinyurl.com/simple-plotting-solutions]] (PDF)

- [X] Written test in class next Monday (on EDA 1st lesson).

- [X] Give a short account of your term project progress now!

** Term projects

Remember:
1) Work on your term projects this Wednesday (no class).
2) Satisfy all assignment requirements.
3) Look at some examples for abstracts (linked).
4) Make sure you have references.
5) Make sure your references are complete and consistent.
6) Make sure that you're reporting on your results (+/-).
7) Make sure you've got 250 words minimum.

* DONE Week 14: DataCamp: EDA Test / EDA for numerical data

- [X] DataCamp review: EDA test - categorical data
- [X] Final presentation (4th sprint review): two new rubrics
- [X] Review: 10 piccolo plotting problems
- [X] Annotating and building stacked/dodged barplots
- [X] Introduction to the fourth (last) sprint review

- [X] *Please fill in the course evaluation now! (5 min)*
- [X] Datalab folder contains the solution to "10 piccolo problems"
- [X] Give an introduction to Minto's Pyramid Principles
- [X] Numerical plots: +Histograms+, boxplots, scatterplots

** EDA test (self-graded)

- Time: 30 minutes.

- Write the R code and the interpretation on paper.

- Experiment at will using R.

- For the interpretation ("what does this mean?"), either make a
  general statement or (if you run code) use a data example.

  Question: Create a summary of the =supp= ~factor~ of the =ToothGrowth=
  data frame (with values "OJ" and "VC"). What does the output mean?
  ...
  If you don't run this as code, you could write:
  #+begin_quote
  Since =supp= is a ~factor~, the output is a table showing how many
  records are in each of the ~levels~ of =supp=.
  #+end_quote

  If you did run the code, you could write:
  #+begin_quote
  The output shows that 30 guinea pigs received a supply (=supp=) of
  Orange Juice (=OJ=) and that 30 received a supply of Vitamin C (=VC=).
  #+end_quote

- Sample data for the test: [[https://tinyurl.com/comics-csv][tinyurl.com/comics-csv]]

** Final presentation

- Schedule your presentation date in Canvas
- Two new rubrics available
- Graded: oral communication skills
- Graded: problem solving skills
- Send me your stuff before if you want a pre-view
- Submit all material to your GDrive no later than 12 Dec

** 10 piccolo plotting exercises with ~mtcars~

- Download the notebook file from here: [[https://tinyurl.com/piccolo-problems][tinyurl.com/piccolo-problems]].
- Let's complete it together to get back in the saddle.
- Let's move on to more plotting practice (code along).
- The Datalab folder now also contains the solutions.

** Storytelling & Presenting with Minto method
#+attr_html: :width 400px :float nil:
[[../img/pyramids.jpg]]

- Main messages of the video:
  #+begin_quote
  1) "You need to tell a story in order to succeed [as a leader]."
  2) Communicating a story is hard.
  3) Minto's "pyramid principle" convinces rational audiences.
  4) Story = S[ituation] + C[omplication] + Q[uestion] + A[nswer].
  5) Presentation = Arguments and evidence supporting the Answer.
  6) What does not support the Answer must be eliminated.
  7) This is not grade school: no points for completeness.
  8) Using this principle will make spreadsheet people love you.
  9) The success of a presentation is about being understood.
  10) You can structure every argument that way.
  #+end_quote

- Additional MECE principle (to structure arguments on each level):
  When separating a concept into subsets, the items should be mutually
  exclusive (ME) and collectively exhaustive (CE).

- Example: Final degrees decompose into: BSc, BA, MSc, MA, PhD.

- Extended example: A University Coffee Shop (Thank you, Gemini)

  1. *Situation* (The Status Quo): The Daily Grind has been the primary
     coffee provider for students and faculty for over 10 years.
     - We possess a loyal customer base.
     - We hold a central location on campus.
     - Revenue has remained steady at $500,000 annually for the last
       three years.

  2. *Complication* (The Disruption): However, a major competitor,
     "Star-Brew," is opening a location across the street next
     semester.
     - Their prices are 10% lower than ours.
     - They offer a mobile ordering app, which we currently lack.
     - /Risk:/ Surveys suggest 40% of our student customers may switch
       for the app's convenience.

  3. *Question* (The Pivot): Given this new competitive threat and our
     lack of digital infrastructure, how can The Daily Grind retain
     its customer base and protect its revenue?

  4. *Answer* (The Main Idea / Solution): We must immediately launch a
     proprietary "Skip-the-Line" mobile app integrated with a loyalty
     program.
     - *Goal:* Neutralize Star-Brew's convenience advantage.
     - *Mechanism:* Leverage existing brand loyalty to lock in
       customers before the competitor opens.

- Challenge: Take 10 minutes to identify SCQA for your project, then
  let's hear some examples and discuss them.

- Why are my lectures not SCQA structured? Actually, the scripts are
  totally structured that way but the delivery is focused on
  interactivity and learning, not on presenting (research) results.


** Review: SCQA Practice Exercise

- Complication and Question often were not aligned:
  1) C: Short rest between games increases stress.
  2) Q: Does higher workload increase injury risk?

  This question just repeats the complication. A better complication
  would be: "It is not clear how in-between game rests and injury risk
  are related."

  This leads to the question: "How are they related". Your
  presentation gives the answer.

- Since your project is finished, keep the Question and fit the
  complication to the question that you already worked on.

- Good questions ask for "HOW" and not "IF":
  1) "Can data analysis reveal patterns...?" worse than
  2) "How can data analysis reveal patterns?"

- Here is a perfect example for an SCQA (Ava Rose):
  #+begin_quote
  S = The Titanic was a tragedy that killed hundreds of people.

  C = There were many uncontrollable factors that influenced one's
  survival rate.

  Q = What were the factors that most influenced the survival rates of
  the Titanic passengers?

  A = The most influential factors were age, gender, and economic
  status.
  #+end_quote

** DataCamp review: Exploring Numerical Data

Open a new datalab notebook to code along during this review.

1) What R function from *ggplot2* creates a dotplot of a single
   numerical variable?

   #+begin_src R :session *R* :results output :exports both
     str(cars) # speed of cars and distance required to stop at that speed
   #+end_src

   #+begin_src R :file plot.png :session *R* :results graphics output file :exports both
     library(ggplot2) # load graphics library `ggplot2`
     data(cars)       # load numeric `cars` dataset
     ggplot(cars,
            aes(x = speed)) +
       geom_dotplot() 
   #+end_src

2) How would you deal with the ~binwidth~ warning?
   #+begin_src R :file plot1.png :session *R* :results graphics output file :exports both
     library(ggplot2)
     data(cars)
     ggplot(cars,
            aes(x = speed)) +
...
3) Does the =count= scale on the y-axis show frequency of observations?
   #+begin_quote
   No. The ~geo_dotplot()~ function scales the height of the stacks
   proportionally. Each dot represents a constant density (a ratio),
   not one observation. To show counts, use ~method="histodot"~ (though
   you may not see a change at all but the dots are now observations).
   #+end_quote

   #+begin_src R :file plot1b.png :session *R* :results graphics output file :exports both
     ggplot(cars,
            aes(x = speed)) +
       geom_dotplot(binwidth=1,
                    dotsize=0.5,
...
4) Compare with a histogram with specific ~binwidth~.
   #+begin_src R :file plot4.png :session *R* :results graphics output file :exports both
     ## The width of each bin, which affects the histogram’s resolution.
...
5) In a *density plot*, what does the curve represent?
   #+begin_src R :file plot5a.png :session *R* :results graphics output file :exports both
     ## Density = smoothed estimate of the distribution of the variable.
     ggplot(cars, aes(x=speed)) +
       geom_density()
   #+end_src
   
   Compare with base R using ~density()~:
   #+begin_src R :file plot5b.png :session *R* :results graphics output file :exports both
     plot(density(x=cars$speed)) # a lot more information and grace
   #+end_src

6) How do you flip the orientation of a boxplot in ggplot?
   #+begin_src R :file plot6a.png :session *R* :results graphics output file :exports both
     ## To flip, add ~coord_flip()~ to the ggplot call.
     ggplot(cars,
            aes(y=speed)) +
       geom_boxplot() +
       coord_flip()
   ...
   Contrast this with base R's ~boxplot()~:
   #+begin_src R :file plot6b.png :session *R* :results graphics output file :exports both
     boxplot(cars$speed,
             ylab="Car speed [mph]")
   #+end_src
...
7) What summary statistics define the box portion of a ~boxplot~?
   #+begin_src R :session *R* :results output :exports both
     ## Boxplot: first quartile, median, and third quartile (the IQR)
     summary(cars)
     IQR(cars$speed) # determines outliers - none in this dataset
   #+end_src

8) What is the difference between a marginal and a conditional
   distribution?
   #+begin_quote
   Marginal uses all data; conditional subsets by a grouping variable.
   #+end_quote

   #+begin_src R :file plot8.png :session *R* :results graphics output file :exports both
     ## Filtering data before plotting
     cars2 <- subset(cars, speed < 15)
     ggplot(cars2, aes(x = speed)) +
       geom_histogram(color="white",fill="blue",binwidth=2)
   #+end_src

9) What ggplot function is used to create faceted plots for comparing
   distributions across groups?
   #+begin_quote
   ~facet_wrap()~ or ~facet_grid()~.
   #+end_quote

   #+begin_src R :file plot9.png :session *R* :results graphics output file :exports both
   ...
   What is shown here?
   #+begin_quote
   - =fast= is a ~logical~ variable we created. It has two ~levels~.
     1. ~FALSE~: ~cars~ with =speed= <= 15
     2. ~TRUE~: ~cars~ with =speed= > 15
   - ~facet_wrap~ makes separate histograms for each value of =fast=.
   - Each panel shows the distribution of stopping distances within
     that subgroup: slow cars on the left, fast cars on the right.
   #+end_quote

10) What does filtering data (e.g., =filter(eng_size < 2.0)=) accomplish
    before plotting?
    #+begin_quote
    Filtering restricts the visualization to a subset of observations,
    or a subset of data frame rows.
    #+end_quote

    #+begin_src R :file plot10.png :session *R* :results graphics output file :exports both
      ## Faceted density plot using two categorical variables
      cars$fast <- cars$speed > 15
      cars$short_stop <- cars$dist < 50

      ggplot(cars, aes(x = speed)) +
        geom_density() +
        facet_grid(fast ~ short_stop)
    #+end_src
    ...
    Explanation:
    1. Upper left panel (fast = FALSE, short_stop = FALSE)
       #+begin_quote
       Slow cars with long stopping distances.
       These cars were not moving fast but still required a long distance to stop,
       which makes them atypical. The density peak around 12–14 mph reflects slow
       cars that stopped surprisingly slowly.
       #+end_quote

    2. Upper right panel (fast = FALSE, short_stop = TRUE)
       #+begin_quote
       Slow cars with short stopping distances.
       This is the normal scenario: low speeds and short braking distances.
       The density here represents the bulk of the dataset’s slower observations.
       #+end_quote

    3. Lower left panel (fast = TRUE, short_stop = FALSE)
       #+begin_quote
       Fast cars with long stopping distances.
       This is expected: higher speeds usually produce longer braking distances.
       The density shows speeds clustered around 17–22 mph.
       #+end_quote

    4. Lower right panel (fast = TRUE, short_stop = TRUE)
       #+begin_quote
       Fast cars with short stopping distances.
       These are unusual observations: cars moving quickly but stopping quickly.
       This panel has fewer cases, with a density hump around 18–22 mph.
       #+end_quote

11) In a side-by-side boxplot, why must the x-variable often be
    converted to a factor?
    #+begin_quote
    Because boxplots require discrete grouping levels on the x-axis.
    #+end_quote

12) In a faceted density plot with two logical variables, what does
    each panel represent?
    #+begin_quote
    The conditional distribution for one combination of the two
    logical variables.
    #+end_quote
...
- [ ] Schedule optional final oral exam in my office for Thu Dec 18.
- [ ] Last quiz with 50 questions on distributed computing (Dec 12).
- [ ] Remember to put the link to your materials into the Canvas page.
- [ ] I will only grade projects whose materials I can access (Dec 12)
