...
1. What does the function ~getwd()~ do in R?
   #+begin_quote
   ~getwd()~ shows the current working directory where R reads and
   writes files.
   #+end_quote

2. How do you change the working directory in R?
   #+begin_quote
   Use the ~setwd()~ function with either a relative or absolute path.
   #+end_quote

3. What is the difference between a relative path and an absolute path in R?
   #+begin_quote
   Relative paths are given from the current location (e.g., =..=),
   while absolute paths start from the computer's root directory
   (e.g., =/home/user=).
   #+end_quote

4. How can you run a shell command such as ~pwd~ from inside R?  
   #+begin_quote
   Use the ~system()~ function, e.g., ~system('pwd')~.
   #+end_quote
   
5. What does the R command ~system("ls -la | wc -l")~ do? What is this
   kind of command called?
   #+begin_quote
   This Unix *pipeline* command lists all files in the directory and
   then counts the number of lines in the listing.
   #+end_quote
